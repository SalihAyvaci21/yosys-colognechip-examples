...
set TOP=ButtonLedToggle
set SRC=src/ButtonLedToggle.v src/debounce_ip_core.v
set VHDL_SRC=
set LOG=0
...
if "%1"=="synth" (
  if %LOG%==1 (
    start /WAIT /B %YOSYS% -l log/synth.log -p "read -sv %SRC%; synth_gatemate -top %TOP% -nomx8 -vlog net/%TOP%_synth.v"
  ) else (
    start /WAIT /B %YOSYS% -ql log/synth.log -p "read -sv %SRC%; synth_gatemate -top %TOP% -nomx8 -vlog net/%TOP%_synth.v"
  )
)
